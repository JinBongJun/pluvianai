...
import { X, Loader2, ChevronLeft, CheckSquare, Square, Activity, Database, ChevronDown, ChevronRight } from 'lucide-react';
...
export function NodeAndDataPickerModal({
  ...
  const datasetsKey =
    open && pickedAgent && projectId && !isNaN(projectId)
  ...
  const recentListKey =
    open && step === 'data' && dataSource === 'recent' && pickedAgent && projectId && !isNaN(projectId)
      ? ['release-gate-recent-list', projectId, pickedAgent.agent_id, RECENT_LIST_LIMIT]
      : null;
  const { data: recentListData, isLoading: recentListLoading, error: recentListError } = useSWR(
    recentListKey,
  ...
  const recentFallbackKey =
    ...
    recentListError
      ...
      : null;
  const { data: recentFallbackData, isLoading: recentFallbackLoading, error: recentFallbackError } = useSWR(
    recentFallbackKey,
    () => liveViewAPI.listSnapshots(projectId, { agent_id: pickedAgent!.agent_id, limit: RECENT_LIST_LIMIT, offset: 0 })
  );

  const recentItemsSource = Array.isArray(recentListData?.items) && recentListData.items.length > 0
    ? recentListData.items
    : (recentFallbackData?.items ?? recentListData?.items ?? []);
  const recentSnapshotsRaw: RecentSnapshotItem[] = recentItemsSource.map((s: { id: string | number; trace_id?: string; created_at?: string }) => ({
    id: String(s.id),
    trace_id: s.trace_id,
    created_at: s.created_at,
  }));
  const recentSnapshots = recentSort === 'oldest' ? [...recentSnapshotsRaw].reverse() : recentSnapshotsRaw;
  ...
  const handleQuickSelectRecent = (n: number) => {
    const ids = recentSnapshotsRaw.slice(0, n).map((s) => s.id);
  ...
  const canConfirm =
    pickedAgent &&
    (dataSource === 'recent' ? tempSnapshotIds.size > 0 : dataSource === 'datasets' && tempDatasetIds.size > 0);

  const handleConfirm = () => {
    if (!pickedAgent || !canConfirm) return;
    if (dataSource === 'recent') {
      onConfirm({ agent: pickedAgent, dataSource: 'recent', snapshotIds: Array.from(tempSnapshotIds) });
    } else {
      onConfirm({ agent: pickedAgent, dataSource: 'datasets', datasetIds: Array.from(tempDatasetIds) });
    }
    onClose();
  };
  ...
  return (
    ...
    >
      <div
        ...
        aria-hidden="true"
      />
      <div className="relative w-full max-w-2xl max-h-[85vh] flex flex-col rounded-2xl border border-white/10 bg-[#111216] shadow-2xl">
        ...
        {step === 'node' && (
          <div className="flex-1 overflow-y-auto min-h-0 p-4 space-y-2">
            <p className="text-xs text-slate-500 mb-3">Choose an agent (Live View node), then pick Live View logs or saved datasets.</p>
            {agentsLoading ? (
              <div className="flex items-center justify-center py-12">
                <Loader2 className="w-8 h-8 animate-spin text-slate-500" aria-hidden />
              </div>
            ) : agents.length === 0 ? (
              <p className="text-sm text-slate-500 text-center py-8">No agents yet. Run flows in Live View to see nodes here.</p>
            ) : (
              agents.map((agent) => (
                <AgentPickerCard
                  key={agent.agent_id}
          ...
          <>
            ...
            <div className="px-5 pt-4 shrink-0">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                >
                  <Activity className={`w-5 h-5 shrink-0 mt-0.5 ${dataSource === 'recent' ? 'text-fuchsia-400' : 'text-slate-500'}`} />
                  <div>
                    <div className="text-sm font-semibold text-slate-100">Live View logs</div>
                    <div className="text-xs text-slate-500 mt-0.5">Use recent runs from this node (no save needed)</div>
                  </div>
                </button>
                <button
                  type="button"
                ...
                >
                  <Database className={`w-5 h-5 shrink-0 mt-0.5 ${dataSource === 'datasets' ? 'text-fuchsia-400' : 'text-slate-500'}`} />
                  <div>
                    <div className="text-sm font-semibold text-slate-100">Saved datasets</div>
                    <div className="text-xs text-slate-500 mt-0.5">Pick from datasets you saved (e.g. Clinical selection)</div>
                  </div>
                </button>
              </div>
            </div>
              ...
              <>
                <div className="px-5 py-3 border-t border-white/5 bg-white/[0.02] shrink-0 flex flex-wrap items-center justify-between gap-2">
                  <span className="text-xs text-slate-400 font-medium">
                    {totalAvailable !== null && (
                      <>This node has <span className="font-semibold text-slate-200">{totalAvailable}</span> recent run{totalAvailable !== 1 ? 's' : ''}. Select which to use:</>
                    )}
                    {totalAvailable === null && !isRecentLoading && 'Select runs from the list below.'}
                ...
                <div className="flex items-center justify-between px-5 py-2 border-t border-white/5 shrink-0 flex-wrap gap-2">
                  <span className="text-xs text-slate-400 font-medium">{tempSnapshotIds.size} selected</span>
                  <div className="flex items-center gap-2">
                    ...
                    >
                      {tempSnapshotIds.size >= recentSnapshots.length ? 'Deselect All' : 'Select All'}
                    </button>
                  </div>
                </div>
                <div className="flex-1 overflow-y-auto min-h-0 p-4 space-y-2" style={{ minHeight: 200 }}>
                  {isRecentLoading ? (
                    <div className="flex items-center justify-center py-12">
                      <Loader2 className="w-8 h-8 animate-spin text-slate-500" aria-hidden />
                    </div>
                    ...
                    <p className="text-sm text-slate-500 text-center py-8">No recent runs for this node. Run flows in Live View to see logs here.</p>
                  ) : (
                        ...
                        >
                          ...
                          <div className="min-w-0 flex-1">
                            <div className="text-sm font-medium text-slate-100 truncate font-mono">
                              {snap.trace_id ? `Trace ${String(snap.trace_id).slice(0, 12)}…` : `Snapshot ${snap.id}`}
                            </div>
              ...
              <>
                ...
                <div className="flex-1 overflow-y-auto min-h-0 p-4 space-y-2" style={{ minHeight: 200 }}>
                  {datasetsLoading ? (
                    ...
                    <p className="text-sm text-slate-500 text-center py-8">No saved datasets for this agent. Save a dataset from Live View (DATA tab).</p>
                  ) : (
                      ...
                      return (
                        <div key={dataset.id} className="space-y-1">
                          ...
                          >
                              ...
                              <div className="min-w-0 flex-1">
                                ...
                                <div className="text-xs text-slate-500 mt-0.5 truncate">
                                  ...
                                  {Array.isArray(dataset.snapshot_ids) && (
                                    <span className="ml-2">· {dataset.snapshot_ids.length} runs</span>
                                  )}
                                </div>
                              </div>
                            </button>
                            <button
                              type="button"
                              onClick={(e) => { e.stopPropagation(); setExpandedDatasetId((id) => (id === dataset.id ? null : dataset.id)); }}
                          ...
                          {isExpanded && (
                            <div className="rounded-lg border border-white/10 bg-black/30 px-3 py-2 ml-4 max-h-48 overflow-y-auto">
                              <div className="text-[10px] uppercase tracking-wider text-slate-500 mb-2">Contents</div>
                              {previewSnapshotsLoading ? (
                                <div className="flex items-center justify-center py-4">
                                  <Loader2 className="w-5 h-5 animate-spin text-slate-500" aria-hidden />
                                </div>
                              ) : previewSnapshots.length === 0 ? (
                                <p className="text-xs text-slate-500">No snapshots in this dataset.</p>
                              ) : (
                                <ul className="space-y-1 text-xs text-slate-300">
                                  {previewSnapshots.slice(0, 20).map((snap: { id?: string; trace_id?: string; created_at?: string }) => (
                                    <li key={snap.id} className="truncate font-mono">
                                      {snap.trace_id ? `Trace ${String(snap.trace_id).slice(0, 12)}…` : `Snapshot ${snap.id}`}
                                      {snap.created_at && <span className="text-slate-500 ml-2">{new Date(snap.created_at).toLocaleString()}</span>}
                                    </li>
                                  ))}
                                  {previewSnapshots.length > 20 && (
                                    <li className="text-slate-500">… and {previewSnapshots.length - 20} more</li>
                                  )}
                                </ul>
                              )}
                            </div>
            ...
            <div className="p-5 border-t border-white/10 shrink-0 flex justify-end gap-3">
              ...
              >
                {dataSource === 'recent' ? `Confirm (${tempSnapshotIds.size} runs)` : `Confirm (${tempDatasetIds.size})`}
              </button>
            </div>
          </>
        )}
