...
type PassRateGaugeProps = {
  ...
  totalRuns?: number;
  size?: number; // Circle size (default: 140)
  strokeWidth?: number;
    ...
    <div className="flex flex-col items-center justify-center">
      <div className="relative" style={{ width: size, height: size }}>
        ...
        <svg width={size} height={size} className="transform -rotate-90">
          ...
          {/* Progress circle or loading arc */}
          <circle
            cx={center}
            cy={center}
            r={radius}
            fill="transparent"
            stroke={`url(#${isValidating ? "grad-loading" : hasResult ? gradientId : "none"})`} // Transparent when no result; loading gradient while validating
            strokeWidth={strokeWidth}
            strokeDasharray={
              isValidating ? `${circumference / 3} ${(circumference * 2) / 3}` : circumference
            }
