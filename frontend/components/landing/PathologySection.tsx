...
const pathologies = [
    {
        id: 'hallucination',
        title: 'Hallucination Virus',
        description: 'Agents confidently inventing facts. The most common cause of deployment failure.',
        icon: Brain,
        color: 'text-red-500',
    ...
    },
    {
        ...
        description: 'Forgetting instructions after 5 turns. Critical logic breakdown in long conversations.',
        icon: FileQuestion,
    ...
    },
    {
        id: 'drift',
        title: 'Tone Drift',
        description: 'Friendly agents becoming rude or weird. A silent killer of user trust.',
        icon: Activity,
...
export default function PathologySection() {
    return (
        <section id="problem" className="py-32 relative bg-transparent overflow-hidden">
            ...
            {/* Background Grid - Clinical Feel (Subtle) */}
                ...
                >
                    {pathologies.map((pathology, index) => (
                        ...
                        >
                            ...
                            <div className={`w-14 h-14 rounded-xl flex items-center justify-center mb-8 bg-white/5 border border-white/10 group-hover:scale-110 transition-transform duration-500 ${pathology.color}`}>
                            ...
                            <div className="absolute right-4 bottom-4 flex flex-col items-end gap-1 opacity-20 group-hover:opacity-100 transition-opacity duration-500">
                                <span className="text-[10px] font-mono text-emerald-500">SYS.DIAG.0{index + 1}</span>
