...
export function AgentPolicyPanel({ projectId, agentId }: { projectId: number; agentId: string }) {
  ...
  const { data: projectRulesData, mutate: mutateProjectRules, isLoading: projectRulesLoading } = useSWR(
    projectId ? ['policy-rules-project', projectId] : null,
    () => behaviorAPI.listRules(projectId, { enabled: true, scope_type: 'project' })
  );
  const { data: agentRulesData, mutate: mutateAgentRules, isLoading: agentRulesLoading } = useSWR(
  ...
  const projectRules = React.useMemo(() => (Array.isArray(projectRulesData) ? projectRulesData : []), [projectRulesData]);
  const agentRules = React.useMemo(() => (Array.isArray(agentRulesData) ? agentRulesData : []), [agentRulesData]);
  const reports = React.useMemo(() => (Array.isArray(reportsData?.items) ? reportsData.items : []), [reportsData]);
  ...
  const handleCreateRule = async (payload: Omit<BehaviorRule, 'id' | 'created_at' | 'updated_at' | 'project_id'>) => {
    await behaviorAPI.createRule(projectId, payload);
    await Promise.all([mutateProjectRules(), mutateAgentRules()]);
    await globalMutate((key) => Array.isArray(key) && key[0] === 'behavior-rules' && key[1] === projectId);
    await globalMutate((key) => Array.isArray(key) && key[0] === 'policy-rules-project' && key[1] === projectId);
    await globalMutate(
      (key) => Array.isArray(key) && key[0] === 'policy-rules-agent' && key[1] === projectId && key[2] === agentId
    );
    setActionStatus(payload.scope_type === 'project' ? 'Project default rule created.' : 'Agent override rule created.');
  };
    ...
    <div className="flex flex-col h-full bg-[#0a0f1e]/20 text-slate-200">
      ...
      <div className="p-6 border-b border-white/5 bg-black/20">
        <div className="flex items-center justify-between gap-4 flex-wrap">
          <div className="flex items-start gap-3">
            ...
            <div>
              <div className="text-[11px] font-black uppercase tracking-[0.24em] text-slate-400">Policy</div>
              <h3 className="text-3xl font-black text-white mt-1">Rules & Checks</h3>
              <p className="text-sm text-slate-400 mt-1">
                Enabled project defaults: {projectRules.length} · enabled agent overrides: {agentRules.length}
              </p>
              {latestReport && (
                <p className="text-[13px] text-slate-400 mt-1">
                  Latest report: <span className="text-slate-300 font-semibold uppercase">{String(latestReport.status)}</span>
                  {latestReport.created_at ? ` · ${formatTraceTime(String(latestReport.created_at))}` : ''}
                </p>
              )}
            </div>
          </div>
          <div className="w-full text-right">
            <p className="text-[11px] text-slate-400">
              Policy evaluation and dataset save actions are handled in Clinical Log.
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar p-6 space-y-4">
        <div className="rounded-2xl border border-white/10 bg-black/20 p-4">
          <div className="text-[11px] font-black uppercase tracking-[0.24em] text-slate-400 mb-2">Status</div>
          <div className="text-sm text-slate-300">
            Configure policy rules here. Run validation from Clinical Log per snapshot.
          </div>
          {latestReport && (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <section className="rounded-xl border border-white/10 bg-black/20 overflow-hidden shadow-2xl">
            <div className="flex items-start justify-between gap-3 p-5 border-b border-white/5 bg-white/[0.01]">
              <div>
                <div className="text-[11px] font-black uppercase tracking-[0.2em] text-slate-500">Scope · Project</div>
            ...
            {isAnyRulesLoading ? (
              ...
              <RuleList rules={projectRules} emptyMessage="No rules yet. Click New Rule to create a baseline guardrail." />
            )}
          </section>
          ...
          <section className="rounded-xl border border-white/10 bg-black/20 overflow-hidden shadow-2xl">
            <div className="flex items-start justify-between gap-3 p-5 border-b border-white/5 bg-white/[0.01]">
              <div>
                <div className="text-[11px] font-black uppercase tracking-[0.2em] text-slate-500">Scope · Agent</div>
                <h4 className="text-base font-bold text-white mt-0.5">Agent Override Rules</h4>
                <p className="text-xs text-slate-500 mt-1">
                  Overrides for {agentId}.
                </p>
              </div>
              <button
            ...
            {isAnyRulesLoading ? (
              ...
              <RuleList rules={agentRules} emptyMessage="No overrides yet. Click New Rule to add an agent-specific policy." />
            )}
          </section>
        </div>
      </div>
