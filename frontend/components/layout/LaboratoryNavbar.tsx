    ...
    return (
        <nav className="flex items-center gap-1.5 h-full">
            {navItems.map((item) => {
                ...
                // Construct safe href
                const href = effectiveProjectId ? `/organizations/${orgId}/projects/${effectiveProjectId}/${item.href.split('/').pop()}` : '#';

                const colorMap: Record<string, { active: string; bg: string; icon: string; border: string }> = {
                    fuchsia: { active: 'text-fuchsia-400', bg: 'bg-fuchsia-500/10', icon: 'text-fuchsia-500', border: 'border-fuchsia-500/20' },
                    ...
                    sky: { active: 'text-sky-400', bg: 'bg-sky-500/10', icon: 'text-sky-500', border: 'border-sky-500/20' }
                };
