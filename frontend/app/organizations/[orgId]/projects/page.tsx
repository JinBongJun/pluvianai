        ...
        {loading && (
          <div className="grid gap-6 sm:grid-cols-2">
            {Array.from({ length: 4 }).map((_, idx) => (
              <div
                key={idx}
                className="h-48 animate-pulse rounded-xl bg-white/5 border border-white/10"
              />
            ))}
          </div>
        )}
            ...
            {filteredProjects.map(p => (
              ...
              >
                ...
                <button
                  ...
                  className="absolute top-3 right-3 z-30 h-8 w-8 rounded-full bg-[#05070c]/80 border border-white/15 flex items-center justify-center text-slate-400 hover:text-white hover:border-emerald-500/40 transition-all"
                ...
                >
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="mb-1.5 flex items-center gap-2">
                      <div className="flex items-center gap-1.5 px-2.5 py-1 bg-white/[0.05] border border-white/10 rounded-full">
                        <span className="text-[10px] font-black text-slate-400 uppercase tracking-[0.16em]">
                          ID:
                        </span>
                        <span className="text-[10px] font-black text-emerald-400 tracking-[0.1em]">
                          {p.id}
                        </span>
                      </div>
                      {(p.alerts ?? 0) > 0 && (
            ...
            <table className="min-w-full text-base text-slate-200">
              <thead className="bg-white/5 text-[11px] font-black uppercase tracking-[0.3em] text-slate-500 border-b border-white/5">
                <tr>
                  ...
                  <th className="px-10 py-6 text-left">Project ID</th>
                    ...
                    <td className="px-10 py-8">
                      <div className="flex flex-wrap items-center gap-2">
                        <button
                          ...
                          className="rounded-lg border border-white/10 bg-white/[0.04] px-3 py-1.5 font-mono text-sm font-semibold text-emerald-400 tabular-nums hover:border-emerald-500/40 hover:bg-emerald-500/10 transition-colors"
                          title="Copy project ID"
                        >
                          {p.id}
                        </button>
                        {(p.alerts ?? 0) > 0 ? (
                          <span className="inline-flex items-center gap-1.5 rounded-full border border-red-500/30 bg-red-500/15 px-2.5 py-1 text-[10px] font-black uppercase tracking-[0.16em] text-red-400">
