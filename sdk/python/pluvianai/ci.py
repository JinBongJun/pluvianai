...
class CIClient:
    """CI client for PluvianAI model validation"""
    ...
    def __init__(
        ...
        self.project_id = project_id or int(os.getenv("PLUVIANAI_PROJECT_ID", "0"))
        self.api_url = api_url or os.getenv("PLUVIANAI_API_URL", "https://api.pluvianai.com")
        self.timeout = timeout
        self.skip_on_failure = skip_on_failure

        if not self.api_key:
            raise ValueError("PLUVIANAI_API_KEY environment variable is required")
        if not self.project_id:
            raise ValueError("PLUVIANAI_PROJECT_ID environment variable is required")
    ...
    def validate(
        ...
        try:
            ...
            if passed:
                print(f"PluvianAI: Model validation passed (score: {score:.2f}, elapsed: {elapsed:.2f}s)")
                return {
                    "status": "passed",
                    "score": score,
                    "elapsed": elapsed,
                ...
                message = f"PluvianAI: Model validation failed (score: {score:.2f}, elapsed: {elapsed:.2f}s)"
                if self.skip_on_failure:
                    print(f"Warning: {message} - Continuing deployment (skip_on_failure=true)")
                    return {
                        "status": "warning",
                        "score": score,
                        "elapsed": elapsed,
            ...
            message = f"PluvianAI: Validation timeout after {self.timeout}s"
            if self.skip_on_failure:
                print(f"Warning: {message} - Continuing deployment (skip_on_failure=true)")
                return {
            ...
            message = f"PluvianAI: Network error: {str(e)}"
            if self.skip_on_failure:
                print(f"Warning: {message} - Continuing deployment (skip_on_failure=true)")
                return {
            ...
            message = f"PluvianAI: Validation error: {str(e)}"
            if self.skip_on_failure:
                print(f"Warning: {message} - Continuing deployment (skip_on_failure=true)")
                return {
    ...
    def run_ci_validation(
        ...
        try:
            ...
            if passed:
                print(f"PluvianAI: CI validation passed (score: {score:.2f}, elapsed: {elapsed:.2f}s)")
                return {
                    "status": "passed",
                    "score": score,
                    "elapsed": elapsed,
                ...
                message = f"PluvianAI: CI validation failed (score: {score:.2f}, elapsed: {elapsed:.2f}s)"
                if skip_on_failure:
                    print(f"Warning: {message} - Continuing deployment (skip_on_failure=true)")
                    return {
                        "status": "warning",
            ...
            message = f"PluvianAI: CI validation timeout after {timeout}s"
            if skip_on_failure:
                print(f"Warning: {message} - Continuing deployment (skip_on_failure=true)")
                return {
                    "status": "timeout_warning",
            ...
            message = f"PluvianAI: CI validation error: {str(e)}"
            if skip_on_failure:
                print(f"Warning: {message} - Continuing deployment (skip_on_failure=true)")
                return {
                    "status": "error_warning",
...
def run_validation(
    ...
    Args:
        project_id: Project ID (or set PLUVIANAI_PROJECT_ID env var)
        api_key: API Key (or set PLUVIANAI_API_KEY env var)
        api_url: API URL (or set PLUVIANAI_API_URL env var)
...
def main():
    """CLI entry point"""
    parser = argparse.ArgumentParser(description="PluvianAI CI Model Validation")
    parser.add_argument("--project-id", type=int, help="Project ID (or set PLUVIANAI_PROJECT_ID)")
    parser.add_argument("--api-key", type=str, help="API Key (or set PLUVIANAI_API_KEY)")
    parser.add_argument("--api-url", type=str, help="API URL (or set PLUVIANAI_API_URL)")
    ...
    try:
        ...
        print(f"PluvianAI CI: Fatal error: {str(e)}")
        sys.exit(1)
