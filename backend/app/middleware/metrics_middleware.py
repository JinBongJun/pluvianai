...
class MetricsMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next: Callable) -> Response:
        ...
        try:
            ...
            if request_size > 0:
                api_request_size_bytes.labels(method=method, endpoint=path).observe(request_size)

            if response_size > 0:
                api_response_size_bytes.labels(method=method, endpoint=path).observe(response_size)
