...
# Canonical step: step_order, step_type ("llm_call" | "tool_call"), tool_name?, tool_args?, + base_meta
...
def _detect_provider(response_data: Any, provider_hint: Optional[str] = None) -> str:
    ...
    if isinstance(candidates, list) and len(candidates) > 0:
        ...
        if isinstance(content, list):
            return "google"
    # Anthropic: top-level "content" array with blocks (type "text" or "tool_use")
    content = response_data.get("content")
    if isinstance(content, list) and len(content) > 0:
        first = content[0] if isinstance(content[0], dict) else {}
        if first.get("type") in ("text", "tool_use"):
            return "anthropic"
    return "unknown"
...
def response_to_canonical_steps(
    ...
    *,
    ...
    """
    Convert provider-specific response to canonical steps (llm_call + tool_call steps).
    ...
    Returns list of step dicts with step_order, step_type, tool_name, tool_args.
    """
    ...
    for i, tc in enumerate(tool_calls):
        step_meta = {**base, "step_order": step_order_base + (i + 1) * 0.01, "step_type": "tool_call", "tool_name": tc.get("name") or "", "tool_args": tc.get("arguments") or {}}
