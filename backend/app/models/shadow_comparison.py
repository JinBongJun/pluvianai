...
class ShadowComparison(Base):
    ...
    primary_api_call = relationship(
        "APICall", foreign_keys=[primary_api_call_id], back_populates="primary_shadow_comparisons"
    )
    shadow_api_call = relationship(
        "APICall", foreign_keys=[shadow_api_call_id], back_populates="shadow_shadow_comparisons"
    )
