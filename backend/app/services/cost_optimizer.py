...
class CostOptimizer:
    ...
    def suggest_optimizations(
        ...
        total_potential_savings = sum(
            o.get("estimated_monthly_savings", 0) for o in safe_opportunities
        )
    ...
    def _analyze_usage_patterns(
        ...
        for model_key, pattern in model_patterns.items():
            pattern["avg_cost_per_call"] = (
                pattern["total_cost"] / pattern["total_calls"]
                if pattern["total_calls"] > 0 else 0.0
            )
            pattern["avg_latency_ms"] = (
                pattern["total_latency"] / pattern["total_calls"]
                if pattern["total_calls"] > 0 else 0.0
            )
            pattern["avg_quality_score"] = (
                sum(pattern["quality_scores"]) / len(pattern["quality_scores"])
                if pattern["quality_scores"] else 75.0
            )
    ...
    def _find_simple_task_opportunities(
        ...
        for pattern in usage_patterns:
            ...
            is_simple_task = (
                avg_quality < 70 and
                avg_latency < 2000 and
                pattern.get("total_input_tokens", 0) / pattern.get("total_calls", 1) < 500
            )
