...
class WebhookService:
    ...
    async def trigger_webhooks(
        ...
        project_webhooks = db.query(Webhook).filter(
            Webhook.project_id == project_id,
            Webhook.is_active == True
        ).all()
        
        # Get user-level webhooks (project_id is None)
        user_webhooks = db.query(Webhook).filter(
            Webhook.user_id == project.owner_id,
            Webhook.project_id.is_(None),
            Webhook.is_active == True
        ).all()
        ...
        if not relevant_webhooks:
            return {
                "triggered": 0,
                "total": 0,
                "message": "No webhooks found for this event type"
            }
        
        results = {
            "triggered": 0,
            "failed": 0,
            "total": len(relevant_webhooks),
            "details": []
        }
    ...
    async def trigger_alert_webhooks(
        self,
        alert: Alert,
        db: Session
    ) -> Dict[str, Any]:
