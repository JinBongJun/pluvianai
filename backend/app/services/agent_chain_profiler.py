...
class AgentChainProfiler:
    ...
    def profile_chain(
        self, project_id: int, chain_id: Optional[str] = None, days: int = 7, db: Optional[Session] = None
    ) -> Dict[str, Any]:
        """
        ...
        query = (
            db.query(APICall)
            .options(joinedload(APICall.quality_scores))
            .filter(
                and_(
    ...
    def _analyze_chain(self, chain_id: str, calls: List[APICall], db: Session) -> Dict[str, Any]:
        ...
        for agent_name, agent_calls in agents.items():
            ...
            for call in agent_calls:
                ...
                if hasattr(call, "quality_scores") and call.quality_scores:
                    ...
                    scores = db.query(QualityScore).filter(QualityScore.api_call_id == call.id).all()
        ...
        if agent_stats:
            ...
            potential_bottlenecks = [
                a for a in agent_stats if a["avg_latency_ms"] > avg_agent_latency * bottleneck_threshold_multiplier
            ]
    ...
    def get_agent_statistics(self, project_id: int, days: int = 7, db: Optional[Session] = None) -> Dict[str, Any]:
        """
