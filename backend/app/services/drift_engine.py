...
class DriftEngine:
    ...
    def detect_drift(
        ...
        current_query = db.query(APICall).filter(
            and_(APICall.project_id == project_id, APICall.created_at >= current_start, APICall.created_at <= now)
        )
    ...
    def _detect_length_drift(
        ...
        baseline_lengths = [len(call.response_text or "") for call in baseline_calls if call.response_text]
        ...
        current_lengths = [len(call.response_text or "") for call in current_calls if call.response_text]
    ...
    def _detect_latency_drift(
        ...
        baseline_latencies = [call.latency_ms for call in baseline_calls if call.latency_ms is not None]
        ...
        current_latencies = [call.latency_ms for call in current_calls if call.latency_ms is not None]
