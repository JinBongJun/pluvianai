...
class AgentChainOptimizer:
    ...
    def suggest_optimizations(self, project_id: int, chain_id: str, db: Session) -> Dict[str, Any]:
        """
        ...
        """
        # 1. 체인 프로파일링
        profile_result = self.profiler.profile_chain(project_id=project_id, chain_id=chain_id, days=7, db=db)
        ...
        # 4. 예상 개선 효과 계산
        predicted_improvement = self._calculate_predicted_improvement(chain_profile, safe_optimizations)
    ...
    def _find_parallel_opportunities(self, chain_profile: Dict[str, Any]) -> List[Dict[str, Any]]:
        """
        병렬화 가능한 에이전트 발견
        ...
        sequential_slow_agents = [a for a in agents if a.get("avg_latency_ms", 0) > avg_latency * 0.5]
    ...
    def _optimize_agent_order(
        self, chain_profile: Dict[str, Any], db: Session, project_id: int
    ) -> List[Dict[str, Any]]:
        """
        에이전트 실행 순서 최적화 제안
        ...
        if current_order != optimized_order:
            ...
            improvement_pct = (
                ((current_sequential_time - optimized_sequential_time) / current_sequential_time * 100)
                if current_sequential_time > 0
                else 0
            )
    ...
    def _calculate_predicted_improvement(
        self, chain_profile: Dict[str, Any], optimizations: List[Dict[str, Any]]
    ) -> Dict[str, Any]:
        """
        예상 개선 효과 계산
