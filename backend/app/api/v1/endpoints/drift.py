...
async def detect_drift(
    ...
    detections = drift_engine.detect_drift(
        project_id=project_id, model=request.model, agent_name=request.agent_name, db=db
    )
...
async def get_drift_detection(
    detection_id: int, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)
):
    """Get a specific drift detection"""
    detection = db.query(DriftDetection).filter(DriftDetection.id == detection_id).first()

    if not detection:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Drift detection not found")
