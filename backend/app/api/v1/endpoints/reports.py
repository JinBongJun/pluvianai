...
def _add_standard_report_content(elements: list, report_data: dict, heading_style, normal_style):
    ...
    intro_style = ParagraphStyle(
        "Intro", parent=normal_style, fontSize=10, textColor=colors.HexColor("#555555"), spaceAfter=12
    )
    elements.append(
        Paragraph(
            "This report provides a high-level overview of key performance metrics for the reporting period.",
            intro_style,
        )
    )
    ...
    elements.append(
        Paragraph(
            "For detailed analysis and breakdowns, please refer to the Detailed Report template.", conclusion_style
        )
    )
...
def _add_detailed_report_content(elements: list, report_data: dict, heading_style, normal_style):
    ...
    elements.append(
        Paragraph(
            "This comprehensive report provides detailed analysis including model performance, provider breakdowns, error analysis, and daily trends for in-depth evaluation.",
            intro_style,
        )
    )
    elements.append(Spacer(1, 0.15 * inch))
    ...
    if quality_scores.get("average") is not None:
        summary_data.append(["Average Quality Score", f"{quality_scores.get('average', 0):.2f}%"])
        summary_data.append(
            ["Quality Score Range", f"{quality_scores.get('min', 0):.1f}% - {quality_scores.get('max', 0):.1f}%"]
        )
    ...
    if breakdown.get("by_model", {}).get("top_models"):
        ...
        intro_style = ParagraphStyle(
            "SectionIntro", parent=normal_style, fontSize=9, textColor=colors.HexColor("#666666"), spaceAfter=10
        )
        elements.append(
            Paragraph(
                "Top 10 models ranked by API call volume, showing usage patterns and performance metrics.", intro_style
            )
        )
        ...
        models_table = Table(
            models_data, colWidths=[2 * inch, 0.75 * inch, 0.75 * inch, 0.75 * inch, 0.75 * inch, 0.75 * inch]
        )
    ...
    if breakdown.get("error_breakdown"):
        ...
        elements.append(
            Paragraph("HTTP status code distribution showing error patterns and failure rates.", intro_style)
        )
        elements.append(Spacer(1, 0.1 * inch))
        ...
        for status_code, count in sorted(error_breakdown.items(), key=lambda x: x[1], reverse=True):
            ...
            errors_data.append([status_code, f"{count:,}", f"{percentage:.2f}%"])
    ...
    if breakdown.get("daily_trends"):
        ...
        elements.append(
            Paragraph(
                "Day-by-day breakdown of API calls and token usage showing usage patterns over time.", intro_style
            )
        )
        elements.append(Spacer(1, 0.1 * inch))
        ...
        for trend in daily_trends[:30]:  # Limit to last 30 days for readability
            ...
            trends_data.append(
                [date_str, f"{trend.get('calls', 0):,}", f"{input_tokens:,}", f"{output_tokens:,}", f"{total_tokens:,}"]
            )
...
def _add_executive_report_content(elements: list, report_data: dict, heading_style, normal_style):
    ...
    elements.append(
        Paragraph(
            "This executive report provides high-level KPIs, performance trends, and strategic recommendations for decision-makers.",
            intro_style,
        )
    )
    elements.append(Spacer(1, 0.15 * inch))
    ...
    if trends and isinstance(trends, dict):
        ...
        elements.append(
            Paragraph("Comparison between first half and second half of the reporting period.", intro_style)
        )
        elements.append(Spacer(1, 0.1 * inch))
        ...
        for trend_key, trend_data in trends.items():
            ...
            trends_data.append([trend_name, f"{int(first_half):,}", f"{int(second_half):,}", change_str, trend_icon])
    ...
    if top_performers and isinstance(top_performers, list) and len(top_performers) > 0:
        ...
        for model in top_performers[:5]:
            ...
            performers_data.append([model_name, f"{int(calls):,}", f"{avg_latency:.2f}"])
    ...
    if recommendations and isinstance(recommendations, list) and len(recommendations) > 0:
        ...
        for rec in recommendations:
            ...
            recs_data.append([priority, rec_type, rec_paragraph])  # Use Paragraph instead of plain string
...
async def _generate_report_data(
    project_id: int, template: str, date_from: Optional[str], date_to: Optional[str], current_user: User, db: Session
) -> dict:
    ...
    successful_calls = api_calls_query.filter(and_(APICall.status_code >= 200, APICall.status_code < 300)).count()
    ...
    drift_query = db.query(DriftDetection).filter(DriftDetection.project_id == project_id)
    if start_date:
    ...
    high_severity_drifts = drift_query.filter(DriftDetection.severity.in_(["high", "critical"])).count()
    ...
    total_cost = sum(
        cost_analyzer.calculate_cost(call.provider, call.model, call.request_tokens or 0, call.response_tokens or 0)
    ...
    if template == "standard":
        ...
        # Get error breakdown
        error_breakdown = db.query(APICall.status_code, func.count(APICall.id).label("count")).filter(
        ...
        for stat in model_stats:
            ...
            model_cost = cost_analyzer.calculate_cost(
                stat.provider, stat.model, stat.total_input_tokens or 0, stat.total_output_tokens or 0
            )
            ...
            **base_report,
            ...
            "summary": {
                ...
                "drift_detections": {
                    "total": drift_count,
                    "high_severity": high_severity_drifts,
                    "critical": db.query(DriftDetection)
                    .filter(DriftDetection.project_id == project_id, DriftDetection.severity == "critical")
                    .filter(
                        *([DriftDetection.detected_at >= start_date] if start_date else []),
                        *([DriftDetection.detected_at <= end_date] if end_date else []),
                    )
                    .count(),
                    "medium": db.query(DriftDetection)
                    .filter(DriftDetection.project_id == project_id, DriftDetection.severity == "medium")
                    .filter(
                        *([DriftDetection.detected_at >= start_date] if start_date else []),
                        *([DriftDetection.detected_at <= end_date] if end_date else []),
                    )
                    .count(),
                    "low": db.query(DriftDetection)
                    .filter(DriftDetection.project_id == project_id, DriftDetection.severity == "low")
                    .filter(
                        *([DriftDetection.detected_at >= start_date] if start_date else []),
                        *([DriftDetection.detected_at <= end_date] if end_date else []),
                    )
            ...
            "breakdown": {
                "by_model": {
                    "top_models": sorted(
                        [{"model": k, **v} for k, v in cost_by_model.items()], key=lambda x: x["calls"], reverse=True
                    )[:10],
                    "cost_distribution": cost_by_model,
                },
                "by_provider": {},
                "error_breakdown": {str(error.status_code): error.count for error in error_breakdown},
                "daily_trends": [
                    {
        ...
        top_models = (
            top_models_query.group_by(APICall.provider, APICall.model)
        ...
        report = {
            **base_report,
            ...
            "key_metrics": {
                ...
                "avg_quality_score": (
                    round(float(quality_scores.avg_score), 2) if quality_scores and quality_scores.avg_score else None
                ),
...
async def generate_report(
    ...
    return await _generate_report_data(
        project_id=project_id, template=template, date_from=date_from, date_to=date_to, current_user=current_user, db=db
    )
