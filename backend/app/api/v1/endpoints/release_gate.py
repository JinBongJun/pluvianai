...
from app.schemas.release_gate import (
    ReleaseGateJobActiveResponse,
    ReleaseGateJobCreateResponse,
    ReleaseGateJobGetResponse,
    ReleaseGateJobOut,
    ReleaseGateJobProgressOut,
...
def _job_to_out(job: ReleaseGateJob) -> ReleaseGateJobOut:
    request_json = getattr(job, "request_json", None)
    request_obj = request_json if isinstance(request_json, dict) else {}
    owner_agent_id = str(request_obj.get("agent_id") or "").strip() or None
    repeat_runs = _coerce_optional_int(request_obj.get("resolved_repeat_runs")) or _coerce_optional_int(
        request_obj.get("repeat_runs")
    )
    snapshot_count = _coerce_optional_int(request_obj.get("resolved_snapshot_count"))
    if snapshot_count is None and isinstance(request_obj.get("snapshot_ids"), list):
        snapshot_count = len(request_obj.get("snapshot_ids") or [])
    if snapshot_count is None and bool(request_obj.get("use_recent_snapshots")):
        snapshot_count = _coerce_optional_int(request_obj.get("recent_snapshot_limit"))
    attempts_total = _coerce_optional_int(request_obj.get("resolved_attempts_total"))
    if attempts_total is None and snapshot_count is not None:
        attempts_total = int(snapshot_count) * max(1, int(repeat_runs or 1))
...
async def validate_release_gate_async(
    ...
    request_json["resolved_repeat_runs"] = max(1, int(payload.repeat_runs or 1))
    request_json["resolved_snapshot_count"] = len(resolved_inputs.snapshots)
    request_json["resolved_attempts_total"] = attempts
