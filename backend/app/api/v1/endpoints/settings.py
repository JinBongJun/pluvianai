...
async def create_api_key(
    key_data: APIKeyCreate, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)
):
    ...
    new_key = APIKey(
        user_id=current_user.id, name=key_data.name, key_hash=hashed_key, is_active=True  # Store hashed version
    )
    db.add(new_key)
    db.commit()
    db.refresh(new_key)
...
async def get_notification_settings(current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    """Get notification settings for current user"""
    from app.models.notification_settings import NotificationSettings as NotificationSettingsModel

    # Get or create notification settings
    notification_settings = (
        db.query(NotificationSettingsModel).filter(NotificationSettingsModel.user_id == current_user.id).first()
    )
...
async def update_notification_settings(
    settings: NotificationSettings, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)
):
