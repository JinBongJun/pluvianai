...
async def get_shadow_routing_suggestions(
    ...
    suggestions = shadow_routing_service.suggest_shadow_models(
        project_id=project_id, primary_model=primary_model, db=db
    )
...
async def apply_shadow_routing(
    ...
    if not request.user_confirmation:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="User confirmation is required to apply shadow routing"
        )
