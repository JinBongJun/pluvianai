...
class TestSuperuserLimitSkip:
    ...
    async def test_superuser_is_not_blocked_by_release_gate_attempt_limit(
        self, async_client, auth_headers, db, test_user, test_project
    ):
        db.add(Subscription(user_id=test_user.id, plan_type="free", status="active"))
        trace = Trace(id="rg-superuser-trace", project_id=test_project.id)
        db.add(trace)
        db.add(
            Snapshot(
        ...
        assert _extract_error_code(response.json()) != "LIMIT_RELEASE_GATE_ATTEMPTS"
