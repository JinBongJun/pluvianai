...
2. [핵심 기능 설계](#2-핵심-기능-설계)
   ...
   - [2.5.1 외부 액션(Tool) 처리](#251-외부-액션tool-처리--live-view--test-lab)
...
#### 2.5.1 외부 액션(Tool) 처리 — Live View / Test Lab

에이전트가 이메일·Slack·전화 등 **외부 액션(tool/function call)**을 호출하는 경우, 다음 원칙으로 표시한다.

| 구분 | 정책 | 결과 표시 |
|------|------|-----------|
| **Live View** | 실제 실행 | **호출 시점** + **호출 결과**(성공/실패)를 결과에 함께 표시. SDK/훅으로 tool 실행 및 응답을 캡처한다. |
| **Test Lab** | **Dry run만** | 실제 전송·실행 없이 **tool 호출 + 내용**만 표시. 예: `send_email` 호출 + 이메일 본문, `post_to_slack` 호출 + 메시지 내용. |

- **Test Lab 결과 예시**: "메일 호출 + 메일 내용" 형태로 노출. 여기서 **끝낼지**, 다음 에이전트로 이어갈지는 사용자가 캔버스(화살표/박스)로 설계한다.
- **활용 대상**:
  - **n8n, MCP 등 자동화 시스템** 사용자: 에이전트가 생성한 이메일/Slack/API 호출 내용을 검증한 뒤, 실제 워크플로에서 실행할지 결정.
  - **기업용 다중 에이전트 체인**: 순수하게 에이전트 여러 개를 순차 호출하는 플로우를 Test Lab에서 실험·검증.
