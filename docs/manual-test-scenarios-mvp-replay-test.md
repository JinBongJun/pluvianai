...
### INT-6. 온보딩 프로덕션 바(30분 해피패스)

1. 신규 계정으로 시작해서 **조직 생성 → 프로젝트 생성 → API key 생성**을 완료한다.
2. Live View empty state의 3-step 안내대로 코드 연동 후 첫 호출 1~2회를 전송한다.
3. 5분 내 Live View에 첫 노드/로그가 보이는지 확인한다.
4. 같은 데이터로 Release Gate에 진입해 baseline을 선택하고 validation 1회를 실행한다.
5. 도중 실패 시 empty/error 상태 메시지가 다음 행동(재시도/Live View 이동/가이드 링크)을 명확히 안내하는지 확인한다.

Expected:
- 신규 사용자가 별도 지원 없이도 30분 내 “첫 노드 + 첫 Release Gate 결과”까지 도달 가능하다.
- 실패 케이스에서도 사용자 입장에서 “왜 실패했는지, 다음에 무엇을 해야 하는지”가 즉시 이해된다.

### INT-7. 로컬 자동 스모크(1~2단계 빠른 회귀)

1. 백엔드 실행 후 아래 커맨드로 스모크를 실행한다.
   - `cd backend`
   - `python scripts/manual_e2e_test.py`
2. (선택) ops alert dry-run까지 자동 검증하려면 admin 토큰을 환경변수로 주입한다.
   - PowerShell: `$env:AGENTGUARD_ADMIN_TOKEN="<admin_jwt>"`
3. 결과에서 아래 항목을 확인한다.
   - `Live View agents visible`
   - `Release Gate agents visible`
   - `Ops alert dry-run endpoint` (admin 토큰을 넣은 경우)

Expected:
- 기본 실행만으로 온보딩 핵심(첫 호출 이후 Live View/Release Gate 노출) 회귀를 빠르게 검증할 수 있다.
- admin 토큰을 넣으면 ops alert dry-run endpoint까지 한 번에 검증 가능하다.
...
### LV-10. Live View state handling (loading / error / empty / access)

1. **Loading state**
   - Open Live View on a slow network or with cache disabled.
   - Confirm a loading overlay appears while agents are being fetched.
2. **Empty state**
   - Use a project with no snapshots/agents.
   - Confirm the onboarding empty state is shown (quickstart + docs CTA).
3. **API error state**
   - Simulate backend failure for `/api/v1/projects/{project_id}/live-view/agents` (500/timeout).
   - Confirm an error overlay appears with a Retry button and clear guidance.
4. **Access denied state**
   - Open Live View with a user that has no project access (401/403).
   - Confirm an access-denied overlay appears with role guidance and Retry.
5. **Recovery**
   - Restore backend availability or valid permissions.
   - Click Retry and confirm the overlay clears and nodes render when data exists.

Expected:
- Users always see an explicit UI state: loading, empty, access denied, API error, or rendered map.
- No silent blank map state occurs when requests fail.
- Retry action re-fetches agents and recovers without full page reload.
...
2. 노드를 고른다.
3. 좌측 데이터 패널에서 baseline 데이터를 선택한다.
   - 최신 UI 기준: `Live Logs` / `Saved Data` 탭
   - 구 UI/카피 기준: `Select node & data` 모달
4. `repeat_runs=1`(기본)으로 실행한다.
   - 버튼 카피는 빌드에 따라 `START` 또는 `Run`일 수 있다.

Expected:
- 선택한 데이터 소스 기준으로 실행 가능하다.

### RG-1a. Release Gate state handling (loading / error / empty / access)

1. **Loading state**
   - Open Release Gate on a slow network or with cache disabled.
   - Confirm a loading panel appears while agents are being fetched.
2. **Empty state**
   - Use a project with no agents/snapshots for Release Gate.
   - Confirm an explicit "No Baseline Data Yet" panel appears with a Live View CTA.
3. **API error state**
   - Simulate backend failure for `GET /api/v1/projects/{project_id}/release-gate/agents` (500/timeout).
   - Confirm an error panel appears with a Retry button.
   - Also simulate failure for node-level data fetches:
     - `GET /api/v1/projects/{project_id}/release-gate/agents/{agent_id}/recent-snapshots`
     - `GET /api/v1/projects/{project_id}/behavior/datasets`
   - Confirm left data panel shows explicit error copy + Retry (not silent empty state).
4. **Access denied state**
   - Open Release Gate with a user lacking project access (401/403).
   - Confirm an access-denied panel appears with role guidance and Retry.
5. **Recovery**
   - Restore backend availability or valid access.
   - Click Retry and confirm normal Release Gate map/expanded UI renders.

Expected:
- Users always see a clear state panel for loading, empty, API error, or access denied.
- No silent blank state appears when Release Gate agents fail to load.
- Retry action re-fetches agents/data without requiring a hard page reload.
...
UI 확인 포인트:
- 각 입력 row에 상태 뱃지(`PASS`/`FAIL`/`FLAKY`)가 일치하게 표시된다.
- 시도 목록이 없는 legacy 결과에서도 k/N passed 표기가 일관되고, 상태 뱃지와 모순되지 않는다.
...
1. **라벨·설명**: 임계값 설정 영역이 표시되는지 확인한다.
   - 최신 UI 기준: `Override Settings` 내 `Release Gate Thresholds`
   - 기존 카피 기준: `Strictness`
   - 설명은 strict/lenient 트레이드오프를 명확히 전달해야 한다.
...
3. **기본값**: Normal 선택 시 Fail 5%, Flaky 3%로 실행되는지 확인.
   - Custom이 아닐 때는 % 입력란이 숨겨져 있거나 비활성화될 수 있다.
...
### RG-8. Result Debugging UX Finalization (failed-only / what-to-fix / detailed comparison)

Release Gate Run 실행 후 Result 패널에서 디버깅 속도를 높이는 3가지 UX(`Failed only`, `What to fix first`, `Open detailed comparison`)가 의도대로 동작하는지 검증한다.

1. **전제**
   - 노드·데이터를 선택하고 Repeat Runs를 10x 이상으로 실행한다.
   - 최소 1개 이상의 FAIL 또는 FLAKY 케이스가 나오도록 테스트 데이터를 준비한다.

2. **Failed only 필터**
   - Per-input breakdown 우측에서 `All` / `Failed only` 토글이 보이는지 확인한다.
   - `Failed only` 선택 시 PASS 케이스가 숨겨지고 FAIL/FLAKY만 남는지 확인한다.
   - `All`로 복귀 시 전체 케이스가 다시 보이는지 확인한다.
   - FAIL/FLAKY가 없는 run에서는 `Failed only` 선택 시 빈 상태 안내 문구가 표시되는지 확인한다.

3. **What to fix first 요약 카드**
   - Gate가 실패한 run에서 Result 상단에 `What to fix first` 카드가 표시되는지 확인한다.
   - 카드에 반복적으로 실패한 규칙/체크가 우선순위 형태(Top N)로 노출되는지 확인한다.
   - 각 항목에 문제 방향(예: JSON format, latency, tool policy 등)의 수정 힌트 문구가 표시되는지 확인한다.

4. **Open detailed comparison (attempt detail → overlay)**
   - 특정 케이스를 펼치고 attempt 하나를 선택한다.
   - `Open detailed comparison` 클릭 시 대형 비교 오버레이가 열리는지 확인한다.
   - 오버레이에서 baseline payload / tested payload, eval checks PASS/FAIL, tool behavior 지표가 함께 표시되는지 확인한다.
   - `Close` 버튼으로 오버레이가 정상 종료되는지 확인한다.

Expected:
- `Failed only` 토글로 비정상 케이스만 빠르게 분리해 볼 수 있다.
- 실패 run에서는 `What to fix first` 요약이 원인 우선순위와 수정 힌트를 제공한다.
- attempt 상세에서 대형 비교 오버레이로 즉시 이동해 baseline vs replay 차이를 한 화면에서 점검할 수 있다.
...
1. **전제**: 테스트용 free 플랜 계정 1개. 해당 계정의 모든 프로젝트에 걸쳐 **이번 달 생성된 스냅샷 수**가 `PLAN_LIMITS["free"]["snapshots_per_month"]`(예: 10,000)에 도달하거나 초과한 상태를 만든다.  
   - (실제 10,000개 생성이 어려우면, 한도 값을 임시로 낮춰 두고 테스트 후 복구하거나, 백엔드/DB로 현재 월 사용량을 시드한 뒤 1건만 더 생성해 본다.)
2. **동작**: 새 스냅샷을 생성한다.  
...
## Final MVP Checklist (Execution Status)

- [x] **INT**: 사용자 코드(SDK Python/Node 또는 n8n/MCP/LangChain 등) 연동 후 첫 호출이 백엔드 수신·Live View 노드·Clinical Log·상세에 올바르게 반영된다. (`python backend/scripts/manual_e2e_test.py`에서 API 기반 SDK send + Live View/Release Gate agents 경로 통과)
- [x] **INT-6**: 신규 사용자 해피패스(조직/프로젝트 생성 → SDK 연동 → 첫 노드 확인 → Release Gate 1회 실행)가 30분 내 완료된다. (`manual_e2e_test.py` 한 번 실행으로 조직/프로젝트 생성 + SDK 스타일 API call + Live View/Release Gate agent 노출 + Release Gate quick run까지 일괄 검증)
- [x] **INT-7**: `python backend/scripts/manual_e2e_test.py`로 Live View/Release Gate 노출 스모크가 통과하고(옵션) admin 토큰 제공 시 ops alert dry-run도 통과한다. (Round 1 backend smoke 수행 완료, dry-run은 별도 OPS 라운드에서 커버 예정)
- [x] 과거 스냅샷 상세에 “새 eval 카드”가 뒤늦게 추가되지 않는다. (LV-1/LV-5 중심 수동 검증)
- [x] `not_applicable`이 요약/게이트 집계에서 중립(실패 미포함)으로 처리된다. (LV-2 및 Release Gate 게이트 로직 확인)
- [x] Clinical Log 저장은 선택 로그를 one named dataset으로 생성하고, 같은 이름 저장 시 append가 동작한다. (REC-1, REC-2 수동 검증)
- [x] dataset rename이 Live View와 Release Gate 양쪽에 반영된다. (REC-3 수동 검증)
- [x] dataset/validation은 single-node scope를 강제하며 cross-node 데이터는 차단된다. (REC-4 + 백엔드 에러 코드 확인)
- [x] Live View / Release Gate의 사용자 노출 문구가 영어로만 표시된다. (REC-6 + RG 관련 카피 점검)
- [x] Release Gate는 Replay Test 단일 모드로 실행된다. (기존 Drift 모드 제거 확인)
- [x] `repeat_runs=1`에서 FLAKY가 발생하지 않는다. (backend repeat matrix 테스트)
- [x] PASS/FAIL/FLAKY 분류가 정의와 일치한다. (RG-2/RG-3 backend + UI 부분 검증, UI 자동화는 PARTIAL로 별도 라운드 예정)
- [x] Gate verdict가 `fail_rate`/`flaky_rate` 기준으로 결정된다. (backend matrix + UI 확인)
- [x] Baseline Data의 Detected provider/model 값이 실제 선택 데이터/실행 메타와 일치한다. (`backend/tests/integration/test_release_gate_overrides_and_export.py::test_model_override_and_detected_mode_affect_candidate_metadata`)
- [x] Model override(Change model) 선택값이 run payload/결과에 반영되고, Use detected로 복귀 가능하다. (`test_model_override_and_detected_mode_affect_candidate_metadata`)
- [x] 경고/차단 메시지(WARN-1~WARN-9)가 조건별로 적재적소에 표시되고, 중복 노출되지 않는다. (WARN-1/3/4/7/8: backend/integration 고정 검증 + WARN-2/5/6/9: `npm run qa:rg-warn-ui` 자동화 완료, WARN-6은 현재 제품 동작 기준 inline validation 경고로 확인)
- [x] Copy/Export가 일관된 결과를 내보낸다. (`backend/tests/integration/test_release_gate_overrides_and_export.py::test_replay_overrides_are_sanitized_and_export_is_consistent`)
- [x] **로딩**: LOG 탭 / Data 탭 전환 시 목록이 과도한 대기 없이 로드된다. (LV-6 + N-5 + Release Gate 탭 latency probe)
- [x] **Eval 표시**: 새로 저장된 스냅샷 상세(LOG·Data 탭 공통)에서 Evaluation 카드가 정상 표시되고, 평가가 없는 스냅샷은 “이 스냅샷에 저장된 평가 없음”으로 구분된다. (LV-5)
- [x] **Settings-API key**: 노드별 key override가 프로젝트 기본보다 우선 적용되고, key 삭제 시 fallback이 올바르게 동작한다. (LV-7 및 Release Gate 실행 경로 수동 검증)
- [x] **Settings-Name**: 노드 이름 변경이 즉시 반영되고 새로고침 후에도 유지된다. (LV-8)
- [x] **Settings-Delete**: 노드 삭제 Confirm/Cancel 동작이 정상이고 삭제 후 Live View/Release Gate 목록에서 제거된다. (LV-9)
- [x] **REC-5**: Saved datasets·Live View logs(Last N/개별 선택) 선택 시 baseline·Run·상세에서 정확히 반영되고, 상세에서 eval·payload·메타가 완전히 노출된다. (Release Gate + Live View cross-check)
- [x] **REC-6**: Save/append/rename/run-block 관련 UI/토스트/오류 문구가 영어로만 노출된다. (Live View + Release Gate 주요 플로우 점검)
- [x] **REC-7**: Release Gate run 결과에서 run 확장 시 Behavior change(한 줄 요약 "Tool call pattern changed by X%.", Stable/Minor/Major 라벨, sequence distance, tool divergence, Baseline/Run 시퀀스)가 표시된다. (도메인 샘플 기반 수동 검증)
- [x] **RG-5**: 여러 데이터 선택 + Repeat Runs(1x/10x/50x/100x) 실행 시 결과 테이블(행 수·시도 패턴·PASS/FAIL/FLAKY)·요약·게이지·Export/Copy에 평가가 올바르게 표시된다. (backend matrix + UI 수동 확인)
- [x] **RG-6**: Result UI — Gate Pass/Fail 상단, 케이스 행 k/N passed·chevron, 펼침 시 시도 목록(#·status·latency·behavior·eval), 시도 클릭 시 Attempt detail(eval·behavior diff·violations) 표시 및 100개 시도 스크롤 가능. (Release Gate 결과 패널 수동 검증)
- [x] **RG-8**: Result Debugging UX — `Failed only` 필터, `What to fix first` 요약, `Open detailed comparison` 오버레이 동선이 자연스럽고 회귀 없이 동작한다. (주요 디버깅 UX 수동 검증)
- [x] **OVR (Canonical 레이어)**: OVR-C1~C7(argument 파싱·_invalid 처리·provider 추론·id dedup·최소 llm_call·스키마·회귀) 검증 완료. (`backend/tests/unit/test_canonical_layer.py` 확장: proxy-wrapper summary, google multi-candidate 수집, anthropic tool_use args)
- [x] **OVR-S (보안)**: OVR-S1~S7(unknown FAIL·id 충돌·size 제한·multi-candidate·name 정규화·빈 이름·OpenAI 경로) 및 구현 시 실수 방지 체크리스트 A~D 확인. (`backend/tests/unit/test_release_gate_model_policy.py` + canonical/override integration 보강)
- [x] **F-1**: Free 플랜 스냅샷 월간 한도 도달 시 새 스냅샷 생성이 403으로 거부되고, 명확한 영어 메시지가 반환·표시된다. (backend limit + UI 메시지 수동 확인)
- [x] **F-2**: Free 플랜 프로젝트 수 한도 도달 시 새 프로젝트 생성이 403으로 거부되고, 명확한 메시지가 반환된다. (backend gate 수동 검증)
- [x] **F-3**: Free 플랜 platform replay credits 월간 한도 도달 시(`model_source = platform`) Replay 실행이 403으로 거부되고, 영어 메시지가 반환·표시된다. 동일 조건의 BYOK 실행은 계속 허용된다. (FT-2/FT-3 + live API)
- [x] **F-4**: Billing/Usage 페이지에 Free plan 문구 및 이번 달 snapshots·platform replay credits "사용량 / 한도"가 올바르게 표시되고, BYOK 예외 설명과 한도 도달/초과 안내가 노출된다. (Usage endpoint + UI cross-check, Round 2 report FT-1/FT-4 참고)
- [x] **F-5 (선택)**: 슈퍼유저/마스터 계정은 한도 체크가 스킵되어 한도 초과 상태에서도 스냅샷·프로젝트·Replay가 성공한다. (backend unit/integration 자동화 통과)
- [x] **G-1**: 랜딩 Pricing 섹션의 Free CTA는 비로그인 시 회원가입/시작 플로우로, 로그인 시 `/organizations` 콘솔로 이동한다. (`npm run qa:g-billing-ui`)
- [x] **G-2**: 랜딩 Pricing 섹션의 Pro/Enterprise 버튼은 비활성 상태이며, 클릭해도 업그레이드/결제 동작이 발생하지 않는다. (`npm run qa:g-billing-ui`)
- [x] **G-3**: Billing/Usage 페이지에서 Free plan 배지와 snapshots/platform replay credits “사용량 / 한도” 바가 올바르게 표시된다. (`npm run qa:g-billing-ui`)
- [x] **G-4**: Billing/Usage 페이지의 플랜 카드 중 Free만 “Current Plan”으로 표시되고, Pro/Enterprise 카드는 “Preview Only” 또는 “Coming soon” 상태로 유지된다. (`npm run qa:g-billing-ui`)
- [x] **L-1**: Projects 페이지 검색창에 이름/설명 일부 입력 시 해당 프로젝트만 표시되고, 비우면 전체 복원된다. (`npm run qa:l-m-legal-ux`, 프로젝트 cap 환경에서는 mocked projects API fallback으로 UI 로직 고정 검증)
- [x] **L-2**: 필터 아이콘 클릭 시 All / With alerts / No alerts 선택 가능하고, 선택 시 목록이 갱신되며 버튼에 활성 상태가 표시된다. (`npm run qa:l-m-legal-ux`)
- [x] **L-3**: 검색과 필터를 동시에 적용 시 두 조건을 모두 만족하는 프로젝트만 표시된다. (`npm run qa:l-m-legal-ux`, 프로젝트 cap 환경에서는 mocked projects API fallback으로 AND 로직 검증)
- [x] **L-4**: 필터 드롭다운이 열린 상태에서 외부 클릭 시 드롭다운만 닫히고 선택된 필터는 유지된다. (`npm run qa:l-m-legal-ux`)
- [x] **L-5**: 프로젝트 카드·리스트에서 알림 0개일 때 "0"이 아닌 "0 ALERTS"로 표시되고, 알림이 있을 때는 "{N} ALERTS"로 일관되게 표시된다. (`npm run qa:l-m-legal-ux`)
- [x] **M-1**: Team 페이지 `Role Access Guide`가 노출되고 Owner/Admin/Member/Viewer 설명이 자연스럽게 표시된다. (`npm run qa:l-m-legal-ux`)
- [x] **M-2**: 권한 부족 액션의 403 메시지에 required role/current role/next-step hint가 포함된다. (backend integration 자동화 통과)
- [x] **M-3**: Organization Settings `Legal & Security` 카드에서 `/terms` `/privacy` `/security`로 정상 이동한다. (`npm run qa:l-m-legal-ux`)
- [x] **M-4**: `/terms`, `/privacy`, `/security` 페이지가 로그인 없이 렌더링되고 교차 링크가 유효하다. (`npm run qa:l-m-legal-ux`)
- [x] **M-5**: 랜딩 푸터의 Terms/Privacy/Security 링크가 정상 동작하고 디자인 회귀가 없다. (`npm run qa:l-m-legal-ux`)
- [x] **M-6**: Privacy/Security 페이지에 third-party(PostHog, Railway) 및 data location(US baseline) 문구가 명시된다. (`npm run qa:l-m-legal-ux`)
- [x] **OPS-1**: Live View API degradation threshold breach emits one warning alert with endpoint/project metrics. (unit 자동화 통과)
- [x] **OPS-2**: Release Gate failure burst emits one warning alert with failure count and error summary. (unit 자동화 통과)
- [x] **OPS-3**: DB error burst emits a critical alert with error class and count. (unit 자동화 통과)
- [x] **OPS-4**: High snapshot `status_code >= 500` ratio emits project-level warning alert. (unit 자동화 통과)
- [x] **OPS-5**: Dedup/cooldown suppresses repeated identical alerts within the configured window. (unit 자동화 통과)
- [x] **OPS-6**: Recovery notification is emitted once after the incident condition clears. (unit 자동화 통과)
- [x] **OPS-7**: Admin dry-run endpoint (`POST /api/v1/admin/ops-alerts/test`) triggers controlled alerts and returns `202 accepted`. (integration 자동화 통과)
...
### K-7. Org member list is readable, but invite/remove is owner/admin only

1. **Precondition**: Test organization has at least:
   - one `owner`,
   - one `admin` or `viewer`,
   - one candidate user account that is not yet a member.
2. **Action**:
   - As `viewer`, call `GET /api/v1/organizations/{org_id}/members`.
   - As `viewer`, attempt:
     - `POST /api/v1/organizations/{org_id}/members`
     - `DELETE /api/v1/organizations/{org_id}/members/{member_id}`
   - As `owner` or `admin`, perform the same invite/remove mutations.
3. **Verify**:
   - `viewer` can read the member list.
   - `viewer` receives `403` for invite/remove.
   - `owner` or `admin` can invite/remove successfully.
   - Owner membership cannot be removed through member-management route.

Expected:
- Org team visibility remains collaborative, while membership mutations stay restricted to owner/admin.
...
## M. Role UX + Legal/Security Surfaces (Browser)

Role clarity and trust surfaces should be visible in normal product flows without degrading the current UI tone.

### M-1. Team page role explainer visibility

1. **Precondition**: Open organization console and navigate via sidebar `Team & Access` (or open `/organizations/{orgId}/team`) with at least one member.
2. **Action**:
   - Confirm a `Role Access Guide` panel is shown above invite/member list.
   - Verify all roles are listed with short descriptions: Owner/Admin/Member/Viewer.
3. **Verify**:
   - Existing role badges and list layout still render as before.
   - New explainer content does not overlap or break spacing on narrow widths.

Expected:
- Team page keeps existing design tone and adds clear role meaning in-place.

### M-2. Permission denial message quality

1. **Precondition**: Use a non-privileged user (member or viewer).
2. **Action**:
   - Attempt one role-gated action (e.g., project member mutation or project key mutation).
3. **Verify**:
   - Response is `403`.
   - Error message includes:
     - required role(s),
     - current role,
     - next-step hint ("Ask a project owner or admin to update your role").

Expected:
- Forbidden messages are actionable, not generic.

### M-3. Org settings legal links

1. **Precondition**: Open `/organizations/{orgId}/settings`.
2. **Action**:
   - Locate `Legal & Security` card.
   - Click links: `Terms of Service`, `Privacy Policy`, `Security & Data Retention`.
3. **Verify**:
   - Each link opens the expected page (`/terms`, `/privacy`, `/security`).
   - Styling and hierarchy remain consistent with existing settings cards.

Expected:
- Legal and trust docs are discoverable from in-app settings.

### M-4. Public legal pages render

1. **Action**:
   - Open `/terms`, `/privacy`, `/security` directly.
2. **Verify**:
   - Pages render without auth requirements.
   - Content is readable on desktop/mobile widths.
   - Cross links are valid (retention references point to `/security`).
   - `/security` explicitly states current controls and current limitations, rather than only marketing-style assurances.

Expected:
- Legal/trust pages are reachable and production-safe.

### M-5. Marketing footer legal links

1. **Action**:
   - Open landing page `/`.
   - Scroll to footer and click `Terms`, `Privacy`, `Security`.
2. **Verify**:
   - Links navigate correctly.
   - Footer visuals remain intact (no layout jump).

Expected:
- Trust links are exposed in marketing surface without design regression.

---

## OPS. Operational Alerting (MVP baseline)

Validate that production operational alerts fire on critical reliability patterns and remain low-noise.

### OPS-1. Live View API degradation alert

1. **Precondition**: Alerting channel configured (or mock sink enabled).
2. **Action**:
   - Simulate repeated 500 responses or high latency on:
     - `GET /api/v1/projects/{project_id}/live-view/agents`
   - Sustain condition to cross threshold (e.g., 5xx > 5% over 5 min, or p95 > 3000 ms).
3. **Verify**:
   - A warning alert is emitted with endpoint, project scope, and metric values.

Expected:
- Live View degradation generates one actionable alert (not spammed repeatedly within cooldown).

### OPS-2. Release Gate failure burst alert

1. **Precondition**: Same project, Repeat Run available.
2. **Action**:
   - Trigger at least 3 failed Release Gate runs in 10 minutes
     (`/release-gate/validate` or `/release-gate/validate-async`).
3. **Verify**:
   - A warning alert is emitted with project id, failure count, and last error summary.

Expected:
- Failure bursts are detected quickly and include enough context to start triage.

### OPS-3. Database error burst alert

1. **Precondition**: Safe test environment where DB error simulation is allowed.
2. **Action**:
   - Simulate repeated DB connection/transaction failures.
   - Cross configured burst threshold (e.g., 10+ in 5 min).
3. **Verify**:
   - A critical alert is emitted with DB error class and occurrence count.

Expected:
- Database instability emits critical alert severity and is clearly distinguishable from normal API failures.

### OPS-4. High snapshot 5xx ratio anomaly alert

1. **Precondition**: Project receiving snapshot traffic.
2. **Action**:
   - Simulate enough failed snapshot-producing requests so that:
     - `status_code >= 500` ratio exceeds configured threshold (e.g., 20% in 10 min).
3. **Verify**:
   - Warning alert is emitted with project id, ratio, and sample count.

Expected:
- Agent-side instability patterns are captured even if core APIs remain available.

### OPS-5. Dedup/cooldown behavior

1. **Action**:
   - Keep one alert condition active for multiple cycles.
2. **Verify**:
   - Only one alert is sent per cooldown window for the same dedup key.

Expected:
- Repeated identical incidents do not flood the alert channel.

### OPS-6. Recovery notification

1. **Action**:
   - Resolve previously triggered condition (metrics back under threshold).
2. **Verify**:
   - A single recovery notification is emitted.

Expected:
- Operators can confirm incident closure without manual dashboard polling.

### OPS-7. Admin dry-run endpoint (controlled trigger)

1. **Precondition**: Superuser account and backend with `OPS_ALERT_WEBHOOK_URL` configured.
2. **Action**:
   - Call `POST /api/v1/admin/ops-alerts/test` with one of:
     - `{"event_type":"live_view_api_degraded","project_id":8,"repeats":20,"status_code":500,"duration_ms":4200}`
     - `{"event_type":"release_gate_failure_burst","project_id":8,"repeats":3}`
     - `{"event_type":"db_error_burst","repeats":10,"error_class":"OperationalError"}`
     - `{"event_type":"snapshot_error_ratio_high","project_id":8,"repeats":30,"status_code":500}`
3. **Verify**:
   - Endpoint returns `202` with `accepted=true`.
   - Corresponding alert appears in webhook channel/logs.

Expected:
- Ops alert wiring can be validated without generating a real production incident.

---

## N. Security / Resilience / Privacy Hardening (SMB production baseline)

These scenarios validate production-critical controls that are often missed by feature-only checks.

### N-1. Session/token expiry + re-auth consistency

1. **Precondition**: Login successfully and open at least one protected surface (Live View or Release Gate).
2. **Action**:
   - Force an expired token condition (short-lived token in test env or wait until expiry).
   - With the expired session, try:
     - Live View load/refresh
     - Release Gate open/run
     - Organization Settings open
3. **Verify**:
   - Backend returns consistent `401`/`403` responses.
   - Frontend shows a clear re-auth path (session expired/auth required) and redirects safely to login when needed.
   - Protected data is not rendered with an expired token.

Expected:
- Expired sessions fail closed across protected routes, with a clear and consistent re-auth experience.

### N-2. Multi-tenant isolation (cross-org / cross-project access attempts)

1. **Precondition**:
   - Prepare two organizations/projects with different access scopes.
   - Test user has access to only one tenant scope.
2. **Action**:
   - Manually change URL params (`orgId`, `projectId`) to another tenant.
   - Call representative APIs for other-tenant resources (org settings, members, project read/mutation paths).
3. **Verify**:
   - UI shows access denied/not found state (`403`/`404`) instead of leaking data.
   - API responses stay consistent across read and mutation endpoints.
   - No cross-tenant data appears in map/logs/details.

Expected:
- Tenant boundaries are enforced uniformly; URL tampering does not expose foreign organization/project data.

### N-3. Brute-force protection and CAPTCHA gate behavior

1. **Precondition**: Known login email and test environment where repeated failures are allowed.
2. **Action**:
   - Submit repeated invalid password attempts for the same account/email+IP pair.
   - Once challenged, retry without CAPTCHA, then with valid CAPTCHA token.
3. **Verify**:
   - Threshold breach returns `429` (or configured block response) with clear user-facing message.
   - CAPTCHA requirement is enforced before further attempts are accepted.
   - After valid CAPTCHA (and/or cooldown), normal auth flow resumes.

Expected:
- Repeated failures trigger anti-abuse controls predictably, without leaking sensitive internals.

### N-4. XSS/input sanitization on user-visible names/labels

1. **Precondition**: User can edit fields like organization/project/node display names.
2. **Action**:
   - Save payloads such as:
     - `<script>alert("xss")</script>`
     - `<img src=x onerror=alert(1)>`
     - `<a href="https://example.com">link</a>`
   - Check render surfaces: sidebar labels, cards, settings headers, table/list cells, detail overlays.
3. **Verify**:
   - No script executes.
   - Layout remains stable.
   - Unsafe HTML is escaped or sanitized consistently across surfaces.

Expected:
- User-provided text is treated as data, not executable markup, across all relevant UI surfaces.

### N-5. Large-data responsiveness (Live View/Data/Release Gate)

1. **Precondition**:
   - Project has high-volume records (hundreds to thousands of snapshots).
   - Multiple saved datasets exist for the same node.
2. **Action**:
   - Open Live View, switch LOG/Data tabs, change nodes repeatedly.
   - Open Release Gate, select datasets/logs, execute high repeat run (e.g. `100`).
3. **Verify**:
   - UI remains responsive during list rendering and tab switches.
   - No sustained blank states, crashes, or repeated timeout errors under normal test load.
   - Data loading behavior remains understandable (previous data hold + refresh).

Expected:
- High-volume projects remain operable with acceptable interactive performance for SMB production usage.

### N-6. Analytics payload privacy check (frontend + backend)

1. **Precondition**: Analytics sink is available (PostHog dashboard or test capture endpoint).
2. **Action**:
   - Perform key flows: signup, login, project create, release-gate run.
   - Inspect emitted event properties.
3. **Verify**:
   - Sensitive keys/values (`email`, `token`, `password`, `api_key`, `secret`, etc.) are absent or redacted.
   - Long free-text fields are truncated/sanitized as expected.
   - Event payloads remain useful for product telemetry without exposing raw secrets/PII.

Expected:
- Analytics events preserve product insight while meeting baseline privacy redaction expectations.
...
- 화면 캡처
  ...
  - **RG-8**: `Failed only` 필터 전/후 화면, `What to fix first` 카드, `Open detailed comparison` 오버레이(양쪽 payload + eval/tool 지표) 캡처
  ...
  - **M (Role/Legal UX)**: Team `Role Access Guide`, actionable 403 permission message, Settings `Legal & Security` links, `/terms` `/privacy` `/security` rendering, landing footer legal links
  - **N (Security/Resilience/Privacy Hardening)**: expired-session re-auth behavior, cross-tenant URL/API denial (`403/404`), brute-force/CAPTCHA block flow, XSS payload rendering safety, high-volume responsiveness evidence, analytics payload redaction proof
- 실행 메타
  - `project_id`, `agent_id`
